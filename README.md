...
docker run -d \
  -p 8000:8000 \
  -e DATABASE_URL=postgresql+asyncpg://postgres:<password>@host.docker.internal:5432/booking \
  -e SECRET_KEY=your-secret-key \
  booking-service
```

### Запуск вместе с PostgreSQL (рекомендуется)

```bash
docker-compose up -d
```

Сервис будет доступен на http://localhost:8000. Документация: http://localhost:8000/docs.
